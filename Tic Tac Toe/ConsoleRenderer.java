...
class ConsoleRenderer implements Renderer {
    ...
    private static final int BUFFER_ROWS = Board.SIZE * ROWS_PER_MARK + (Board.SIZE - 1) +
            NUM_ROWS_BEFORE_BOARD;
    private static final int BUFFER_COLS = Board.SIZE * COLS_PER_MARK + (Board.SIZE - 1) +
    ...
    public ConsoleRenderer() {
        ...
        //draw indices
        char index = '1';
        for (int col = NUM_COLS_BEFORE_BOARD + COLS_PER_MARK / 2; col < BUFFER_COLS;
             col += COLS_PER_MARK + 1)
            buffer[NUM_ROWS_BEFORE_BOARD / 2][col] = index++;
        index = '1';
        for (int row = NUM_ROWS_BEFORE_BOARD + ROWS_PER_MARK / 2; row < BUFFER_ROWS;
             row += ROWS_PER_MARK + 1) {
